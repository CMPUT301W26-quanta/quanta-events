...
const createCoInvitationInterface = util.standardForm(
	z.object({
		eventId: z.uuid(),
		invitee: z.uuid(),
	}),
...
export async function createCoInvitation(request: CallableRequest) {
	const { userId, deviceId, data } = util.parseInterface(
		createCoInvitationInterface,
		request,
	);
	...
	logger.info(
		`Creating co-organizer invite ${notificationId} on event ${data.eventId} to user ${data.invitee}.`,
	);
	...
	try {
		await notificationRef.create(
			util.enforceFull<NotificationDocument>({
				...
				message: "You were invited to be a co-organizer for this event!",
